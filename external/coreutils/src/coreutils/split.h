...
public:
    ...
    StringSplit(
        std::basic_string<Char> text, const char delim, int minSplits = -1)
        : source(std::move(text)), ptr(&source[0]), minSplits(minSplits)
    {
...
inline StringSplit<Char> split(
    std::basic_string<Char> const& s, S const& delim, int minSplits = -1)
{
...
template <typename Char, typename S>
inline StringSplit<Char> split(
    const Char* const& s, S const& delim, int minSplits = -1)
{
...
template <size_t N>
auto gen_tuple(const StringSplit<char>& ss)
{
...
{
    auto it = std::search(
        std::begin(text), std::end(text), std::begin(sep), std::end(sep));
