...
//        MRB_ARGS_NONE());
    ...
    {
        self->audio_handler = block;
    });

    mrb::add_method<RAudio>(ruby, "set_freq", [](RAudio *self, int chan, float freq)
    {
        self->channels[chan].step = static_cast<float>(freq) / 44100.F;
    });

    mrb::add_class_method<RAudio>(ruby, "load_wav", [](std::string const &fname)
    {
